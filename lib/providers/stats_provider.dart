...
final workoutStatsProvider = FutureProvider<WorkoutStats>((ref) async {
  try {
    ...
    final results = await Future.wait([
      ...
      analyticsService.getLongestStreak(),
      analyticsService.getTotalRepsThisWeek(),
      analyticsService.getTotalRepsLastWeek(),
      analyticsService.getWorkoutsThisWeek(),
      analyticsService.getTotalTrainingMinutes(),
      analyticsService.getTotalLifetimeReps(),
      analyticsService.getAverageFormScore(),
      analyticsService.getLastWorkoutDate(),
    ]).timeout(
      const Duration(seconds: 10),
      onTimeout: () {
        print('⏱️ Stats loading timeout - returning empty stats');
        throw TimeoutException('Stats loading timeout');
      },
    );
