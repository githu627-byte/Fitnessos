...
class WorkoutPreset {
  ...
  /// Get estimated calories using science-based MET formula
  /// Requires user's bodyweight for accuracy  
  int getEstimatedCalories(double userWeightKg) {
...
class LockedWorkout {
  ...
  factory LockedWorkout.fromPreset(WorkoutPreset preset, {List<WorkoutExercise>? customExercises}) {
    ...
    print('🔍 LockedWorkout.fromPreset DEBUG:');
    print('   Preset: ${preset.name}');
    print('   Total exercises: ${(customExercises ?? preset.exercises).length}');
    print('   Included exercises: ${includedExercises.length}');
    for (var ex in includedExercises) {
      print('     - ${ex.name} (${ex.id}) - included: ${ex.included}');
    }
